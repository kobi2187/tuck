...
- [ ] **[repro] `Seq` bracket indexing (`xs[i]`/`xs[i] = v`) silently
  ...
  the nonexistent `seq_at`.

  **Attempted and reverted (2026-09-04): making the bracket sugar imply
  `seq` automatically.** Injected `seq` as an unconditional implicit
  `dkImport` on every program's entry module in `compiler/modules.nim`'s
  `loadProgram`/`loadProgramIndexed` (as a real decl, so the existing
  emission machinery picks it up like any other import — a bare "load
  seq.tuck as a sibling module" isn't enough, since the Nim `import` line
  and the `entry module last` ordering both come from the decl actually
  being on the entry's own AST, not from the file merely being reachable —
  found the hard way, first attempt appended it after the entry and broke
  "entry last," which several suites depend on to find `fn main`). Fixed
  that, then hit a SECOND real bug it exposed: once `seq` is genuinely
  qualified (`ctx.realModules` contains it), `genQualified` emits
  `seq.setAt(xs, ...)` — and Nim's own builtin `seq[T]` identifier wins
  that lookup, UFCS-expanding it into `setAt(seq, xs, ...)` with the
  builtin passed as a VALUE argument, a baffling type mismatch. Fixed
  THAT too (added `nimModuleAlias` in `codegen_common.nim`, unconditionally
  prefixing every module alias with `tuck_mod_` in both the `import`
  statement and every qualified call site — per the same "cannot clash in
  any backend, now or later" reasoning `mangle.nim` already applies to
  every user-declared global, generalized here to module aliases).

  **Then the REAL blocker surfaced**: making `seq` unconditional broke 28
  assertions across `interface_seq`/`known_bugs`/`loop_var_type`/
  `object_composition` — not because anything was functionally wrong, but
  because dozens of `t.frozen` golden tests compare emitted Nim BYTE FOR
  BYTE, and every single program now carries an extra `import seq as
  tuck_mod_seq` line whether or not it ever indexes a `Seq`. The fix must
  be CONDITIONAL on actual bracket-index usage, not blanket — but that
  information (does this program's AST contain a `Seq`-typed bracket
  index) only exists AFTER typecheck assigns types, while module loading
  (where the import closure is built) runs BEFORE typecheck even starts.
  Real fix direction, not yet attempted: defer the decision to CODEGEN
  time instead of module-load time — when `genQualified`/`genModuleImports`
  actually encounters a `seq::at`/`seq::setAt` qualified call and finds
  `seq` missing from `ctx.realModules`, load `std/seq.tuck` there, on
  demand, rather than deciding eagerly for every program up front. Fully
  reverted (`git checkout -- compiler/{codegen,codegen_common,codegen_emit,
  modules}.nim examples/`), confirmed `./tests/run` green again before
  moving on. The `nimModuleAlias` mangling fix is real and worth keeping
  once the eager-injection problem is solved — the collision it fixes
  (`seq` vs. Nim's builtin) is genuine and reproducible on its own, tested
  in isolation via an explicit `import seq` before the blanket-injection
  attempt made it visible everywhere.
