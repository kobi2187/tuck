...
note anymore). Of the run-checked ones, 14 of 14 now pass.
One example remains uncompiled:
  ...
  - [x] **FIXED 2026-09-01, same day — real `impl: d` support landed.**
        34-ffi-cstring's `zlibVersion` was `impl: nim "...", odin "..."`
        with no `d` entry — genuinely missing, not a bug. Added
        `genDImplFwd`/`implMods` to codegen_d.nim (mirrors
        `genImplForwarders` in codegen_odin.nim: a forwarder calling
        `<alias>.<name>(...)`). The one real design difference from Nim/
        Odin: a D `import` is a bare module name, not a path, so there is
        nothing to rebase or copy — `tuck.nim`'s D build step adds the
        shim's directory as an extra dmd `-I` instead, computed from the
        pre-rebase tree exactly like the existing `.c`-object handling
        for `externLib`. `examples/34-ffi-cstring.tuck` gained a
        `d "./shim/zlib_shim"` entry; `examples/shim/zlib_shim.d` written
        (D's own FFI spelling: `pragma(mangle, ...)` +
        `fromStringz(...).idup`). Verified: prints the real libz version,
        exit 0. All 14 run-checked examples in the sweep now pass —
        0 exclusions.
