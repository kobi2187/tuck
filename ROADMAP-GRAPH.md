...
| **F27** | **who frees a heap value, per backend** | ✗ | nothing in the spec — no section on memory, ownership or lifetimes, and its ONE mention of `GC` (§7.4, "nothing is ever lost, even under GC") assumes a collector rather than specifying one. Nim gets it from ARC, D from its GC, Odin from nothing: `tuckrt/tuck_rt.odin` contains no `delete` or `free` at all (`KNOWN-BUGS-EVENTS.md` EV-12) | **the Odin backend, and the whole freestanding pitch.** Measured: 200k orders through `benches/apps/matching_engine.tuck` leak 5.5 GB on Odin where Nim and D use 21 MB. Adjacent to `F10` (`[no_alloc]`) and `F13` (bare profile), which both assume an allocation story the language never wrote down. `V`'s autofree is the reference worth studying — it frees what it can prove and *leaks the residue* rather than freeing wrongly, which is the right failure direction |
